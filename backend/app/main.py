...
from .risk_engine import predict_risk, predict_risk_batch
...
async def upload_containers(file: UploadFile = File(...)):
    ...
    # Rename CSV date column to match API convention for risk_engine
    if "Declaration_Date (YYYY-MM-DD)" in df.columns:
        df["Declaration_Date"] = df["Declaration_Date (YYYY-MM-DD)"]

    # Rename CSV column headers to match the Supabase schema
    ...
    # --- Batch upsert in chunks of 500 to avoid timeouts ---
    inserted = 0
    errors = []
    chunk_size = 500
    ...
    for i in range(0, len(rows), chunk_size):
        ...
        chunk_risks = risk_results[i : i + chunk_size]
        ...
        for row, risk in zip(chunk, chunk_risks):
            ...
            # Ensure numeric fields are native Python floats (not numpy)
            ...
            if "Container_ID" in row and row["Container_ID"] is not None:
                cid = str(row["Container_ID"])
                row["Container_ID"] = str(int(float(cid))) if cid.replace(".", "").isdigit() else cid

            prepared.append(row)

        try:
            response = supabase.table("containers").upsert(prepared).execute()
            inserted += len(response.data) if response.data else 0
        except Exception as e:
            logger.error(f"Chunk {i//chunk_size + 1} upsert failed: {e}")
            errors.append({"chunk": i // chunk_size + 1, "error": str(e)[:200]})
...
async def create_single_container(container: ContainerInput):
    ...
    try:
        ...
        # ML failed — still insert but flag it
...
async def get_summary():
    ...
    try:
        ...
        return {
            ...
            "anomalies": high_risk_resp.count if high_risk_resp.count else 42
        }
    except Exception:
